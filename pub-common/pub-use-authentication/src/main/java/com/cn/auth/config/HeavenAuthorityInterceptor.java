 ...
 *
 */
public class HeavenAuthorityInterceptor extends HandlerInterceptorAdapter {

    private Logger log= LoggerFactory.getLogger("HeavenAuthorityInterceptor");
    ...
    protected Logger logger = LoggerFactory.getLogger(HeavenAuthorityInterceptor.class);
    private TokenProvider tokenProvider;
    private UserContext userContext;
    private NamedThreadLocal<Long> startTimeThreadLocal = new NamedThreadLocal<Long>("StopWatch-StartTime");

    public HeavenAuthorityInterceptor(TokenProvider tokenProvider, RedisCache redisCache) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        String jwt = request.getHeader(TokenProvider.AUTHORIZATION_HEADER_HEAVEN);
        ...
        //校验jwt是否合法
        if (!jwt.startsWith("BearerHeaven")) {
            //格式不对
            log.info("格式不对");
